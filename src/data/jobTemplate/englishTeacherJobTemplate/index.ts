export const englishTeacherJobTemplate = {
  breadcrumb: {
    category: { en: "Applications", hi: "आवेदन", slug: "/applications" },
    subcategory: {
      en: "Job Applications",
      hi: "नौकरी आवेदन",
      slug: "/applications/job",
    },
    template: {
      en: "English Teacher Job Application",
      hi: "अंग्रेजी शिक्षक नौकरी आवेदन",
      slug: "",
    },
  },

  title: {
    en: "English Teacher Job Application",
    hi: "अंग्रेजी शिक्षक नौकरी आवेदन",
  },

  subtitle: {
    en: "English teacher job application formats with examples, tips, and guidance for school and coaching teaching positions.",
    hi: "स्कूल और कोचिंग शिक्षण पदों के लिए अंग्रेजी शिक्षक नौकरी आवेदन प्रारूप, उदाहरण और मार्गदर्शन।",
  },

  category: {
    en: "Job Applications",
    hi: "नौकरी आवेदन",
  },

  updatedDate: "January 2026",

  editorVariations: [],

  variations: [
    {
      id: "english",
      title: {
        en: "English Teacher Job Application",
        hi: "अंग्रेजी शिक्षक नौकरी आवेदन",
      },
      description: {
        en: "Format for English teaching roles focusing on communication and language skills.",
        hi: "अंग्रेजी शिक्षण पदों के लिए प्रारूप।",
      },
      bestFor: {
        en: "English subject teachers",
        hi: "अंग्रेजी विषय शिक्षक",
      },
      length: {
        en: "Medium",
        hi: "मध्यम",
      },
      difficulty: {
        en: "Easy",
        hi: "आसान",
      },
      content: {
        hi: `सेवा में,
प्रधानाचार्य महोदय/महोदया
[विद्यालय का नाम]
[विद्यालय का पता]
[नगर, राज्य – पिन कोड]

विषय: अंग्रेज़ी शिक्षक पद हेतु आवेदन

माननीय महोदय/महोदया,

मैं आपके विद्यालय में अंग्रेज़ी शिक्षक के पद हेतु आवेदन कर रहा/रही हूँ। मैंने यह रिक्ति विज्ञापन [प्रकाशन/जॉब पोर्टल का नाम] में देखा था। मैंने [विश्वविद्यालय का नाम] से अंग्रेज़ी साहित्य में [डिग्री] प्राप्त की है तथा अंग्रेज़ी शिक्षण का [X] वर्षों का अनुभव रखता/रखती हूँ। मुझे विद्यार्थियों में भाषा कौशल विकसित करने और साहित्य के प्रति रुचि जगाने में विशेष आनंद मिलता है।

मैंने विभिन्न कक्षाओं के छात्रों को अंग्रेज़ी पढ़ाने का कार्य किया है। मैं सरल व स्पष्ट पाठ योजनाएँ तैयार करता/करती हूँ, जिससे छात्रों के व्याकरण, पठन, लेखन एवं वार्तालाप कौशल में सुधार होता है। सीखने की प्रक्रिया को रोचक एवं व्यावहारिक बनाने के लिए मैं समूह पठन सत्र, रचनात्मक लेखन कार्यशाला जैसी गतिविधियाँ भी आयोजित करता/करती हूँ।

मैं मेहनती, सहनशील हूँ तथा एक सकारात्मक एवं सहायक शैक्षिक वातावरण निर्मित करने के प्रति समर्पित हूँ। मुझे आपके विद्यालय में योगदान देते हुए छात्रों को अंग्रेज़ी में आत्मविश्वास प्रदान करने में प्रसन्नता होगी।

मैंने अपना बायोडाटा एवं प्रमाणपत्र संलग्न किए हैं। मैं आपकी सुविधानुसार साक्षात्कार हेतु उपलब्ध रहूँगा/रहूँगी और इस अवसर पर आपसे चर्चा करने की आशा करता/करती हूँ।

आपके समय एवं विचार हेतु धन्यवाद।

भवदीय,
[आपका हस्ताक्षर]
[आपका नाम]

संलग्नक:
1. बायोडाटा
2. B.A./M.A. अंग्रेज़ी साहित्य प्रमाणपत्र
3. B.Ed. प्रमाणपत्र
4. अनुभव पत्र
5. पाठ योजना (Sample Lesson Plans)
6. छात्र उपलब्धि रिकॉर्ड`,
        en: `To,
The Principal
[School Name]
[School Address]
[City, State - PIN Code]

Subject: Application for English Teacher Position

Respected Sir/Madam,

I am writing to apply for the position of English Teacher in your school. I saw the advertisement in [Name of the publication/job portal].

I have a  [degree name] with [xno of year] experience . and I have experience teaching English to students of different classes. I prepare simple and clear lesson plans that help students improve their grammar, reading, writing, and speaking skills. I also conduct activities like group reading sessions and small creative writing workshops to make learning more interesting and practical.

I am hardworking, patient, and dedicated to creating a positive and supportive learning environment. I would be happy to contribute to your school and help students gain confidence in using English.

I have attached my resume and certificates for your reference. I am available for an interview at your convenience and look forward to the possibility of discussing my application with you.

Thank you for your time and consideration.

Yours sincerely,
[Your Signature]
[Your Name]

Enclosures:
1. Resume
2. B.A./M.A. English Literature Certificates
3. B.Ed. Certificate
4. Experience Letters
5. Sample Lesson Plans
6. Student Achievement Records`,
      },
    },
  ],

  blogArticles: [
    {
      title: {
        en: "How to Write a Perfect Teacher Job Application",
        hi: "एक आदर्श शिक्षक नौकरी आवेदन कैसे लिखें",
      },
      excerpt: {
        en: "Complete guide with examples and tips to create an impressive teacher job application that gets noticed.",
        hi: "प्रभावशाली शिक्षक नौकरी आवेदन बनाने के लिए उदाहरण और टिप्स के साथ पूर्ण गाइड।",
      },
      readTime: {
        en: "5 min read",
        hi: "5 मिनट पढ़ें",
      },
      link: "/blog/perfect-teacher-application",
    },
    {
      title: {
        en: "Important Skills to Mention as a Teacher",
        hi: "एक शिक्षक के रूप में उल्लेख करने के लिए महत्वपूर्ण कौशल",
      },
      excerpt: {
        en: "Essential teaching skills and qualities that schools look for in candidates. Boost your application.",
        hi: "आवश्यक शिक्षण कौशल और गुण जो स्कूल उम्मीदवारों में तलाशते हैं।",
      },
      readTime: {
        en: "4 min read",
        hi: "4 मिनट पढ़ें",
      },
      link: "/blog/teacher-skills",
    },
    {
      title: {
        en: "Common Mistakes in Teacher Job Applications",
        hi: "शिक्षक नौकरी आवेदनों में सामान्य गलतियाँ",
      },
      excerpt: {
        en: "Avoid these common errors that can cost you the teaching job. Learn what NOT to do in your application.",
        hi: "इन सामान्य त्रुटियों से बचें जो आपको शिक्षण नौकरी की कीमत चुका सकती हैं।",
      },
      readTime: {
        en: "6 min read",
        hi: "6 मिनट पढ़ें",
      },
      link: "/blog/teacher-application-mistakes",
    },
  ],

  howToWrite: {
    title: {
      en: "Tips to Write an English Teacher Job Application",
      hi: "अंग्रेजी शिक्षक नौकरी आवेदन लिखने के टिप्स",
    },
    tips: [
      {
        text: {
          en: "Mention your English degree and total teaching experience",
          hi: "अपनी अंग्रेजी डिग्री और कुल शिक्षण अनुभव का उल्लेख करें",
        },
      },
      {
        text: {
          en: "Highlight student performance in grammar, writing, and speaking",
          hi: "व्याकरण, लेखन और बोलने में छात्रों के प्रदर्शन को उजागर करें",
        },
      },
      {
        text: {
          en: "Explain how you improve students’ communication skills",
          hi: "बताएं कि आप छात्रों के संचार कौशल कैसे सुधारते हैं",
        },
      },
      {
        text: {
          en: "Mention curriculum experience such as CBSE, ICSE, or State Board",
          hi: "CBSE, ICSE या स्टेट बोर्ड अनुभव का उल्लेख करें",
        },
      },
      {
        text: {
          en: "Include achievements like debates or public speaking coaching",
          hi: "डिबेट या पब्लिक स्पीकिंग कोचिंग जैसी उपलब्धियाँ जोड़ें",
        },
      },
      {
        text: {
          en: "Share examples of creative writing or reading activities",
          hi: "रचनात्मक लेखन या पढ़ने की गतिविधियों के उदाहरण दें",
        },
      },
      {
        text: {
          en: "Attach resume, English certificates, B.Ed., and experience letters",
          hi: "रिज्यूमे, अंग्रेजी प्रमाणपत्र, B.Ed. और अनुभव पत्र संलग्न करें",
        },
      },
      {
        text: {
          en: "Proofread carefully for grammar, spelling, and formatting",
          hi: "व्याकरण, वर्तनी और फॉर्मेटिंग की सावधानीपूर्वक जाँच करें",
        },
      },
      {
        text: {
          en: "Keep language professional and student-focused",
          hi: "भाषा पेशेवर और छात्र-केंद्रित रखें",
        },
      },
    ],
  },

  faqs: [
    {
      question: {
        en: "Which format is best for English teachers?",
        hi: "अंग्रेजी शिक्षकों के लिए कौन सा प्रारूप सबसे अच्छा है?",
      },
      answer: {
        en: "English Teacher format highlights language skills and classroom outcomes.",
        hi: "यह प्रारूप भाषा कौशल और कक्षा परिणाम दर्शाता है।",
      },
    },
    {
      question: {
        en: "Should I mention speaking or writing improvement?",
        hi: "क्या बोलने या लिखने में सुधार लिखना चाहिए?",
      },
      answer: {
        en: "Yes, communication progress strengthens your application.",
        hi: "हाँ, संचार सुधार आवेदन को मजबूत करता है।",
      },
    },
    {
      question: {
        en: "Can freshers apply for English teacher jobs?",
        hi: "क्या फ्रेशर अंग्रेजी शिक्षक के लिए आवेदन कर सकते हैं?",
      },
      answer: {
        en: "Yes, highlight education, internships, and teaching practice.",
        hi: "हाँ, शिक्षा और अभ्यास दिखाएँ।",
      },
    },
    {
      question: { en: "Is B.Ed mandatory?", hi: "क्या B.Ed अनिवार्य है?" },
      answer: {
        en: "Most schools require B.Ed; coaching centers may be flexible.",
        hi: "अधिकांश स्कूलों में B.Ed जरूरी है।",
      },
    },
    {
      question: {
        en: "Should I add demo class experience?",
        hi: "क्या डेमो क्लास अनुभव जोड़ना चाहिए?",
      },
      answer: {
        en: "Yes, it improves credibility.",
        hi: "हाँ, इससे विश्वसनीयता बढ़ती है।",
      },
    },
  ],

  relatedTemplates: [
    {
      title: {
        en: "Application for Science Teacher",
        hi: "विज्ञान टीचर के लिए आवेदन",
      },
      description: {
        en: "Format for Science teaching roles",
        hi: "विज्ञान शिक्षण पदों के लिए प्रारूप",
      },
      icon: "FileText",
      link: "/applications/job/science-teacher-job-application",
    },
    {
      title: {
        en: "Application for Experienced Teacher Job",
        hi: "अनुभवी शिक्षक नौकरी के लिए आवेदन",
      },
      description: {
        en: "For teachers with 2+ years of experience",
        hi: "2+ वर्षों के अनुभव वाले शिक्षकों के लिए।",
      },
      icon: "Briefcase",
      link: "/applications/job/experienced-teacher-job-application",
    },

    {
      title: {
        en: "Application for Mathematics Teacher Position",
        hi: "गणित टीचर पद के लिए आवेदन",
      },
      description: {
        en: "Template for Mathematics teacher role",
        hi: "गणित टीचर भूमिका के लिए टेम्पलेट",
      },
      icon: "GraduationCap",
      link: "/applications/job/math-teacher-job-application",
    },
  ],
};
