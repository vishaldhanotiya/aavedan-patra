export const schoolTransfer = {
  breadcrumb: {
    category: { en: "Applications", hi: "आवेदन", slug: "/applications" },
    subcategory: {
      en: "Admission Applications",
      hi: "प्रवेश आवेदन",
      slug: "/applications/admission",
    },
    template: {
      en: "Student Admission Application",
      hi: "छात्र प्रवेश आवेदन",
      slug: "",
    },
  },
  title: {
    en: "School Transfer Application Letters",
    hi: "स्कूल स्थानांतरण आवेदन पत्र",
  },

  subtitle: {
    en: "Find ready-made formats for transfer from one school to another, school leaving certificate request, TC application, and class transfer within the same school.",
    hi: "एक स्कूल से दूसरे स्कूल में स्थानांतरण, स्कूल लीविंग सर्टिफिकेट हेतु आवेदन, टीसी आवेदन और उसी स्कूल में कक्षा स्थानांतरण के लिए तैयार आवेदन प्रारूप।",
  },

  category: {
    en: "Admission Applications",
    hi: "प्रवेश आवेदन",
  },

  updatedDate: "January 2025",

  variations: [
    {
      id: "schoolTransfer",
      title: {
        en: "School Transfer Application",
        hi: "स्कूल ट्रांसफर आवेदन",
      },
      description: {
        en: "Application for requesting Transfer Certificate (TC).",
        hi: "टीसी के लिए स्कूल ट्रांसफर आवेदन।",
      },
      bestFor: {
        en: "Relocation, school change",
        hi: "स्थानांतरण, स्कूल परिवर्तन",
      },
      length: { en: "Medium", hi: "मध्यम" },
      difficulty: { en: "Easy", hi: "आसान" },

      content: {
        en: `To,
The Principal
[School Name]
[Address]

Subject: Request for Transfer Certificate

Respected Sir/Madam,

I respectfully request the issuance of a Transfer Certificate for my ward, [Student Name], who is currently studying in Class [Class]. We are relocating to [Reason / New City/Location], due to which continuing education at your institution will not be possible.

My child has been a student of your esteemed school for the past [X] years and has shown consistent academic progress and discipline. I kindly request you to issue the Transfer Certificate along with the required documents (marksheet, conduct certificate, etc.) at the earliest to facilitate a smooth admission process at the new school.

I shall be grateful for your cooperation.

Thank you.

Sincerely,
[Parent/Guardian Name]
[Contact Number]
[Date]`,

        hi: `प्रति,
प्रधानाचार्य महोदय/महोदया
[विद्यालय का नाम]
[पता]

विषय: स्थानांतरण प्रमाण पत्र प्रदान किए जाने के संबंध में

महोदय/महोदया,

सविनय निवेदन है कि मेरा पुत्र/पुत्री [Student Name], कक्षा [Class] का नियमित विद्यार्थी/विद्यार्थिनी है। हमारे परिवार का [Reason / स्थानांतरण का कारण] के कारण अन्य शहर/स्थान पर स्थानांतरण हो रहा है। अतः मेरे बच्चे की शिक्षा निर्बाध रूप से जारी रखने हेतु स्थानांतरण प्रमाण पत्र की आवश्यकता है।

आपके प्रतिष्ठित विद्यालय में अध्ययन के दौरान मेरे बच्चे ने पिछले [X] वर्षों में उत्तम आचरण, अनुशासन एवं संतोषजनक शैक्षणिक प्रगति प्रदर्शित की है। कृपया स्थानांतरण प्रमाण पत्र के साथ आवश्यक अभिलेख—

1. पिछली कक्षा का अंकपत्र,
2. चरित्र प्रमाण पत्र,
3. बोनाफाइड प्रमाण पत्र (यदि आवश्यक हो)

प्रदान करने की कृपा करें, जिससे नए विद्यालय में प्रवेश प्रक्रिया समय पर पूर्ण की जा सके।

आपकी सहृदयता एवं सहयोग के लिए मैं आपका आभारी/आभार व्यक्त करता/करती हूँ।

सादर,
[Parent/Guardian Name]
[संपर्क नंबर]
[तारीख]`,
      },
    },
  ],

  howToWrite: {
    title: {
      en: "Tips to Write a School Transfer Application",
      hi: "स्कूल स्थानांतरण आवेदन लिखने के टिप्स",
    },
    tips: [
      {
        text: {
          en: "Mention the student's name, class, roll number, admission number, and current academic session.",
          hi: "विद्यार्थी का नाम, कक्षा, रोल नंबर, प्रवेश संख्या और वर्तमान शैक्षणिक सत्र लिखें।",
        },
      },
      {
        text: {
          en: "State the transfer reason clearly, such as relocation, parent job transfer, or admission to another school.",
          hi: "स्थान परिवर्तन, अभिभावक का तबादला या दूसरे स्कूल में प्रवेश जैसे ट्रांसफर कारण को स्पष्ट लिखें।",
        },
      },
      {
        text: {
          en: "Request the transfer certificate, school leaving certificate, conduct certificate, and marksheet if needed.",
          hi: "यदि आवश्यकता हो तो ट्रांसफर सर्टिफिकेट, स्कूल लीविंग सर्टिफिकेट, आचरण प्रमाणपत्र और मार्कशीट मांगें।",
        },
      },
      {
        text: {
          en: "Attach fee clearance, parent ID proof, new address proof, or new school admission proof if available.",
          hi: "यदि उपलब्ध हो तो फीस क्लीयरेंस, अभिभावक पहचान प्रमाण, नया पता प्रमाण या नए स्कूल का प्रवेश प्रमाण संलग्न करें।",
        },
      },
      {
        text: {
          en: "Ask for quick processing if the new school's admission deadline is near.",
          hi: "यदि नए स्कूल की प्रवेश अंतिम तारीख पास हो तो शीघ्र प्रक्रिया का अनुरोध करें।",
        },
      },
    ],
  },

  relatedTemplates: [
    {
      title: {
        en: "Bonafide Certificate Application",
        hi: "बोनाफाइड प्रमाणपत्र आवेदन",
      },
      description: {
        en: "Format for requesting a bonafide certificate.",
        hi: "बोनाफाइड प्रमाणपत्र प्राप्त करने का प्रारूप।",
      },
      icon: "FileText",
      link: "/templates/bonafide-certificate",
    },
    {
      title: {
        en: "Character Certificate Application",
        hi: "चरित्र प्रमाणपत्र आवेदन",
      },
      description: {
        en: "For school/college character certificates.",
        hi: "स्कूल/कॉलेज चरित्र प्रमाणपत्र के लिए आवेदन।",
      },
      icon: "Briefcase",
      link: "/templates/character-certificate",
    },
    {
      title: { en: "TC Application Letter", hi: "टीसी आवेदन पत्र" },
      description: {
        en: "Transfer certificate application format.",
        hi: "टीसी आवेदन का प्रारूप।",
      },
      icon: "GraduationCap",
      link: "/templates/transfer-certificate",
    },
  ],

  blogArticles: [
    {
      title: {
        en: "How to Write a Perfect School Transfer Application",
        hi: "परफेक्ट स्कूल स्थानांतरण आवेदन कैसे लिखें",
      },
      excerpt: {
        en: "A complete guide to writing strong school transfer applications for TC and migration.",
        hi: "टीसी और माइग्रेशन के लिए स्कूल स्थानांतरण आवेदन लिखने की संपूर्ण गाइड।",
      },
      readTime: { en: "5 min read", hi: "5 मिनट पढ़ें" },
      link: "/blog/write-school-transfer-application",
    },
    {
      title: {
        en: "Documents Required for School Transfer",
        hi: "स्कूल स्थानांतरण हेतु आवश्यक दस्तावेज",
      },
      excerpt: {
        en: "Checklist for TC, migration certificate, and school leaving certificate.",
        hi: "टीसी, माइग्रेशन सर्टिफिकेट और स्कूल लीविंग सर्टिफिकेट हेतु दस्तावेज सूची।",
      },
      readTime: { en: "4 min read", hi: "4 मिनट पढ़ें" },
      link: "/blog/school-transfer-documents",
    },
  ],
  faqs: [
    {
      question: {
        en: "How long should a school transfer application be?",
        hi: "स्कूल स्थानांतरण आवेदन कितना लंबा होना चाहिए?",
      },
      answer: {
        en: "One page is enough. Keep the content clear and mention the reason for transfer briefly.",
        hi: "एक पेज पर्याप्त है। सामग्री स्पष्ट रखें और स्थानांतरण का कारण संक्षेप में बताएं।",
      },
    },
    {
      question: {
        en: "What documents are needed for school transfer?",
        hi: "स्कूल स्थानांतरण के लिए किन दस्तावेजों की आवश्यकता होती है?",
      },
      answer: {
        en: "Original TC, fee clearance certificate, bonafide certificate, and parent ID proof.",
        hi: "मूल टीसी, फीस जमा प्रमाणपत्र, बोनाफाइड प्रमाणपत्र और माता-पिता का पहचान पत्र।",
      },
    },
    {
      question: {
        en: "Can I apply for school transfer online?",
        hi: "क्या मैं ऑनलाइन स्कूल स्थानांतरण के लिए आवेदन कर सकता हूँ?",
      },
      answer: {
        en: "Many schools now accept online transfer applications. Check your school's website for specific process.",
        hi: "अब कई स्कूल ऑनलाइन स्थानांतरण आवेदन स्वीकार करते हैं। विशिष्ट प्रक्रिया के लिए अपने स्कूल की वेबसाइट देखें।",
      },
    },
  ],
};
