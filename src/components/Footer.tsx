      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-8 mb-12">
          ...
          {footerLinks.map((section, index) => (
            <div key={index}>
              <h4 className="text-slate-900 dark:text-white mb-4">{section.title}</h4>
              <ul className="space-y-3">
                {section.links.map((link, linkIndex) => (
                  <li key={linkIndex}>
                    <a 
                      href="#"
                      className="text-slate-600 dark:text-white/60 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors duration-300"
                    >
                      {link}
                    </a>
                  </li>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          >
            Made with <Heart className="w-4 h-4 text-rose-500 fill-rose-500" /> by Vishal Dhanotiya
          </motion.p>
        </div>
      </div>
    </footer>
